...
const InitializeDataButton = () => {
  const [initialized, setInitialized] = useState(hasInitializedData());
  
  const handleInitialize = () => {
    const result = initializeVehicleData();
    setInitialized(true);
    if (result) {
      toast.success("Données de démonstration initialisées avec succès");
    } else {
...
const Index = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (standardVehicles.length === 0 && featuredVehicles.length === 0 && !hasInitializedData()) {
        toast.info("Aucun véhicule trouvé. Utilisez le bouton en bas à droite pour initialiser les données de démonstration.");
      }
    } catch (error) {
